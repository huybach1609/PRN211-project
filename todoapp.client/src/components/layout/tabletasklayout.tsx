...
const TableTaskLayout = ({ title = 'tasks', tasksQuery = [] }: TableTaskLayoutProps) => {
    const [tasks, setTasks] = useState<ITask[]>([]);
    const [taskIdSelected, setTaskIdSelect] = useState<number>(0);

    useEffect(() => {
        setTasks(tasksQuery);
    }, [tasksQuery])

    const getColor = (date: Date | string): string => {
    ...
    const handleRightBar = (task: ITask) => {
        setOpen(true);
    ...
    const handleTaskCreatedOrUpdated = (newTask: ITask, action: 'update' | 'delete' = 'update') => {
        if (action === 'delete') {
    ...
    return (
        <DefaultLayout rightBar={<TaskLayout isOpen={isOpen} setOpen={setOpen} taskId={taskIdSelected} onTaskSaved={handleTaskCreatedOrUpdated} />}>
            <div className="border-[2px] border-bg5 h-[96%] rounded-2xl w-full p-5  gap-1 flex flex-col mr-2">
                <p className="font-bold text-5xl pb-4 text-neutral-700">{title}</p>
