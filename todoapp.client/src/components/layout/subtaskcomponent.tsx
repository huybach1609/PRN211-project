...
interface SubtaskComponentProps {
    subtasksImport?: ISubTask[];
    taskId: number;
}

export const SubtaskComponent = ({ subtasksImport = [], taskId }: SubtaskComponentProps) => {
    const [subtasks, setSubtasks] = useState<ISubTask[]>([]);
    const [newSubtask, setNewSubtask] = useState<{ id: number; name: string }>({ id: 0, name: '' });
    ...
    const addSubtask = () => {
        ...
        fetchAddSubTask(request).then(response => {
            if (newSubtask.name) {
                setSubtasks([
                    ...subtasks,
                    { id: response.data.subTask.id, name: response.data.subTask.name, taskId: taskId, status: true }
                ]);
    ...
    return (
        <div className="w-full">
            ...
            <div className="m-2 flex items-center cursor-pointer" onClick={() => {
                ...
                <Input
                    ...
                    onValueChange={(value) => setNewSubtask({ ...newSubtask, name: value })}
