...
import { CreateListAxios, GetListAccount, UpdateListAxios } from "../services/listservice";
...
export const ListTaskView = () => {
    ...
    const [selectedItem, setSelectedItem] = useState({ id: 0, name: '', tasks: [] });
    useEffect(() => {
        GetListAccount()
            .then(response => {
        ...
        UpdateListAxios(selectedItem)
            .then(response => {
                if (response.data.status) {
                    ...
                    addToast({ title: response.data.message, color: "Failed" });
                }
            })
            .catch();
        onClose();
                ...
                <div className="w-full flex justify-end my-2 px-2">
                    <Dropdown >
                        ...
                        <DropdownMenu aria-label="Static Actions">
                            <DropdownItem key="new" onPress={() => handleOpen(selectedItem, true)}>Add</DropdownItem>
                        ...
                        {(onClose) => (
                            <>
                                ...
                                <ModalBody>
                                    ...
                                    {selectedItem.tasks.map(taskItem => (
