...
export const GetNumOfTaskInfo = async (timestamp, listId) => {
    return axios.get(API_URL + `/count-info/${timestamp}/${listId}`, {
        headers: getHeaders()
    });
}

export const UpdateListAxios = async (selectedItem) => {
    return await axios.put(API_URL + `/odata/Lists`, {
        id: selectedItem.id,
...
export const CreateListAxios = async (selectedItem) => {
    const user = JSON.parse(getUser());
    return await axios.post(API_URL + `/odata/Lists`, {
        name: selectedItem.name,
        accountId: user.id
    }, {
        headers: getHeaders(),
